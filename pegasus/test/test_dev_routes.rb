  ...
  describe '/api/dev/ routes' do
    ...
    describe '/api/dev/start-build' do
      ...
      before do
        ...
        CDO.stubs(:slack_start_build_token).returns(FAKE_SLACK_SLASH_TOKEN)
    ...
    describe 'api/dev/set-last-dtt-green' do
      before do
        ...
        CDO.stubs(:slack_set_last_dtt_green_token).returns(FAKE_SLACK_SLASH_TOKEN)
      end
    ...
    describe 'api/dev/check-dts' do
      ...
      before do
        $log.level = Logger::ERROR
        CDO.stubs(:github_webhook_secret).returns(FAKE_GITHUB_WEBHOOK_SECRET)
      end
